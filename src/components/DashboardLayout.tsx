...
export function DashboardLayout({
    ...
    const handleRefresh = (data?: any, action?: string) => {
      ...
      if (
        ...
        mutate(
          (key: any) =>
            typeof key === "string" &&
            (key.startsWith("/api/dashboard") || key.startsWith("/api/spaces")),
          undefined,
          { revalidate: true },
        );
      }
  ...
  const uploadFileWithPath = useCallback(
    async (file: File, targetFolderId: string | null, uploadId: string) => {
  ...
  const processDroppedItems = useCallback(
    async (dataTransfer: DataTransfer) => {
      if (!spaceId) {
        toast.error(
          "Veuillez entrer dans un espace pour uploader des fichiers.",
        );
        return;
      }
        ...
        <nav className="flex-1 px-6 py-8 space-y-2 overflow-y-auto custom-scrollbar">
          {[
            ...
            },
            {
              icon: Star,
              label: "Favoris",
              href: "/main?filter=favorites",
              filter: "favorites",
            },
            {
              icon: Clock,
              label: "Récents",
              href: "/main?filter=recents",
              filter: "recents",
            },
          ].map((item, i) => {
        ...
        <div className="p-6 border-t border-white/5">
          <div className="flex items-center gap-3 p-2 rounded-2xl group transition-all">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-[#A0A0A0] font-medium truncate">
                {userEmail || "Membre Premium"}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-4 relative">
            <div className="relative">
              ...
              >
                ...
                <InteractiveIconWrapper>
                  <ChevronDown
                    className={`w-3 h-3 text-[#A0A0A0] group-hover:text-white transition-all ${showUserMenu ? "rotate-180" : ""}`}
                  />
                </InteractiveIconWrapper>
              </button>
